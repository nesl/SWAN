...
from .deform_fusion_module import MlvlPointGenerator, MultiScaleDeformableAttention3D
...
class MMIM(nn.Module):
    ...
    def cam_transform(self, cam_volume_feat, volume_coors, batch_mask, img_metas):
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            ...
            lidar2imgs = torch.stack(lidar2img, dim=0).to(cam_volume_feat.device)
    ...
    def fuse(self, volume_feats):
        ...
        for i in range(self.num_encoder_levels):
            ...
            padding_mask_resized = feat.new_zeros((batch_size,)+volume_shape, 
                                                  dtype=torch.bool)
            pos_embed = torch.transpose(self.postional_encoding(padding_mask_resized), 1, 2)
    ...
    def forward(self, volume_feats, lidar_x, img_metas):
        # 14GB GPU memory, without AMP is 18794MiB
