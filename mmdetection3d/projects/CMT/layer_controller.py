...
# This is the Controller that allocates layers among modalities in accordance to modality quality
@MODELS.register_module()
class ADMNConvLayerController(nn.Module):
    
    # Total layer refers to the total available compute budget
    # TODO CHANGE THIS back to 256
    def __init__(self, embed_dim=128, num_classes=8, total_layers_img=12, total_layers_lidar=8, layer_budget=4):
        super(ADMNConvLayerController, self).__init__()
        ...
        self.additional_layers = layer_budget-2

        self.output_head = nn.Sequential(
            nn.Linear(embed_dim, 64),
            nn.ReLU(),
            nn.Linear(64, total_layers_img + total_layers_lidar) # 12 layers in each ViT, we want to generate a one-hot at the end
        )
        self.noise_output = nn.Sequential(
            nn.Linear(embed_dim, 64),
            nn.ReLU(),
    ...
    def forward(self, voxel_features, controller_coors, raw_image, flatformer_layers, temp=1, discretization_method = 'admn'):
        ...
        if discretization_method == 'admn':
            if self.training:
                gumbel_samples = gumbel_softmax_sample(logits, temperature=temp)
            else: # If this is during inference, we don't do any gumbel softmax sampling
                gumbel_samples = logits
            discretized = get_top_k(gumbel_samples, k=self.additional_layers, zero_value=0)
            discretized = torch.reshape(discretized, (B, -1))
...
class UniversalConvLayerController(nn.Module):
    ...
    def __init__(self, embed_dim=128, num_classes=8, total_layers_img=12, total_layers_lidar=8, layer_budgets=[4, 6, 8, 10]):
        ...
        self.logits_memory = [] # Used during inference only
        self.grad_accum = None
        self.register_buffer('mask_buffer', torch.zeros((1, 20), dtype=torch.int32))

    def get_top_k_inplace(self, x, k=8):
        # 2. Use the existing memory (no new allocation)
        # We use [:] to keep the reference to the original buffer
        self.mask_buffer.zero_() 
        
        # 3. Compute indices directly on GPU
        top_k_indices = torch.topk(x, k, dim=1).indices
        # 4. In-place scatter into the pre-allocated buffer
        self.mask_buffer.scatter_(1, top_k_indices, 1)
    ...
    def forward(self, voxel_features, controller_coors, raw_image, flatformer_layers, temp=1, discretization_method = 'neuralsort'):
        ...
        if discretization_method == 'admn':
            ...
            if self.training:
                ...
                if logits.shape[0] == 1:
                    weights = self.get_top_k_inplace(logits, k=current_budget)
                else:
...
class UniversalConvLayerController_New(nn.Module):
    ...
    # TODO CHANGE THIS back to 256
    def __init__(self, embed_dim=128, num_classes=8, total_layers_img=12, total_layers_lidar=8, layer_budgets=[4, 6, 8, 10]):
        super(UniversalConvLayerController_New, self).__init__()

        self.voxel_extractor = nn.Sequential(
            nn.Conv1d(128, out_channels=64, kernel_size=6, stride=6),
            nn.ReLU(),
            nn.Conv1d(64, out_channels=1, kernel_size=6, stride=6),
            nn.ReLU(),
            nn.Linear(138, embed_dim//2)
        )

        self.img_extractor = nn.Sequential(
            nn.Conv2d(3, out_channels=64, kernel_size=3, stride=3),
            nn.ReLU(),
            nn.Conv2d(in_channels=64, out_channels=1, kernel_size=(4, 8), stride=(4, 8)),
            nn.ReLU(),
        )
        self.img_adapter = nn.Linear(288, embed_dim//2)
        self.layer_budgets = layer_budgets

        self.budgets_to_idx = {val: i for i, val in enumerate(layer_budgets)}

        self.layer_tokens = nn.Parameter(torch.randn(len(layer_budgets), embed_dim))
        ...
        self.logits_memory = [] # Used during inference only
        self.grad_accum = None
        self.weights = torch.ones(1, 20)

    def pad_or_truncate(self, tensor, target_size=5000):
        n = tensor.shape[0]
        if n >= target_size:
    ...
    # Temperature define peakiness of the gumbel softmax
    def forward(self, voxel_features, controller_coors, raw_image, flatformer_layers):
        ...
        # budget_token = self.layer_tokens[self.budgets_to_idx[current_budget]]
        # budget_token = budget_token.repeat(B, 1)
    
        # if self.training:
        #     message_hub = MessageHub.get_current_instance()
        #     current_epoch = message_hub.get_info('epoch') + 1
        #     coors = coors.long()
        #     _, num_per_batch = torch.unique(coors[:, 0], sorted=False, return_counts=True)
        #     batch_start_indices = F.pad(torch.cumsum(num_per_batch, dim=0), (1, 0))
        #     voxel_batches = [self.pad_or_truncate(v) for v in voxel_batches]
        #     voxel_features = torch.stack(voxel_batches, dim=0)
        # else:
        #     voxel_features = self.pad_or_truncate(voxel_features).unsqueeze(0)
        
        # voxel_noise_features = self.voxel_extractor(torch.transpose(voxel_features, 1, 2))[:, 0] # Squash middle dim
        
        # raw_image = rearrange(raw_image, 'b n c h w -> (b n) c h w')
        # raw_image = raw_image[:, :, ::4, ::4] # Downsample by factor of 4, now 80 x 200
        # img_noise_features = self.img_extractor(raw_image)
        # img_noise_features = torch.reshape(img_noise_features, (B, -1))
        # img_noise_features = self.img_adapter(img_noise_features)
        ...
        # #     print('Controller LiDAR', weights[0][:flatformer_layers])
        # #     print('Controller Image', weights[0][flatformer_layers:])
        self.weights.copy_(torch.zeros(B, 20).cuda())
        self.weights[:, 0:16] = 1
        predicted_noise = None
        joint_embed = None
        return self.weights, predicted_noise, joint_embed
