...
class CmtDetectorCompiled(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(CmtDetectorCompiled, self).__init__(**kwargs)

        # compile the module
        compile_mode = "reduce-overhead"
        if getattr(self, 'with_pts_backbone', False):
            print("Compiling pts_backbone...")
            self.pts_backbone = torch.compile(self.pts_backbone, mode=compile_mode)
        
        # if getattr(self, 'with_pts_neck', False):
        #     print("Compiling pts_neck...")
        #     self.pts_neck = torch.compile(self.pts_neck, mode=compile_mode)
        
        print("Compiling img_backbone...")
        self.img_backbone = torch.compile(self.img_backbone, mode=compile_mode)

        print("Compiling img_neck...")
        self.img_neck = torch.compile(self.img_neck, mode=compile_mode)

        print("Compiling bbox_head...")
        self.pts_bbox_head.transformer = torch.compile(self.pts_bbox_head.transformer, mode=compile_mode)
    ...
    def init_weights(self):
        ...
        super(CmtDetectorCompiled, self).init_weights()
