...
from .cmt_utils import normalize_bbox
...
class CmtHead(BaseModule):
    ...
    def _rv_pe(self, img_feats, img_metas):
        ...
        if 'pad_shape' in img_metas[0]:
            ...
            pad_h, pad_w = img_metas[0]['img_shape'][:2]
    ...
    def _get_targets_single(self, gt_bboxes_3d, gt_labels_3d, pred_bboxes, pred_logits):
        ...
        def task_assign(bbox_pred, logits_pred, gt_bboxes, gt_labels, num_classes):
            ...
            assign_results = self.assigner.assign(bbox_pred, logits_pred, gt_bboxes, gt_labels, train_cfg=self.train_cfg)
            gt_instances, pred_instances = InstanceData(
            bboxes=gt_bboxes), InstanceData(priors=bbox_pred)
            sampling_result = self.sampler.sample(assign_results, pred_instances, gt_instances)
    ...
    def _dn_loss_single_task(self,
        ...
        map_known_indice = mask_dict['map_known_indice'].long().cuda()
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        with torch.autocast('cuda', enabled=False):
            ...
            for i in range(num_samples):
                preds = preds_dicts[i]
                bboxes = preds['bboxes']
                # Decoded bbox (cx, cy, cz, w, l, h, rot, vx, vy) old format
                ...
                ret_list.append(temp_instances) # Pretty sure we need to convert this to a InstanceData object??
            return ret_list
