...
custom_imports = dict(
    imports=['projects.BEVFusion.bevfusion','projects.CMT','projects.UniM2AE'], allow_failed_imports=False)
...
dataset_type = 'NuScenesCorruptDiverseSplitDataset'
data_root = '/data/HangQiu/data/nuscenes/'
corruption_root = '/data/HangQiu/data/multicorrupt/day'  # Root directory where corrupted data is stored
...
severity_distribution = {1:0.5, 3:0.5}  # Only sample severity 3
Split = 'day'  # 'day', 'night', 'rain', 'dry', None (None means sampling from the whole set, ignoring the split)
corruptions =  ['dark']
data_prefix = dict(
...
# runtime settings
train_cfg = dict(by_epoch=False, max_iters=300, val_interval=100)
val_cfg = dict()
test_cfg = dict()
...
optim_wrapper = dict(
    #type='AmpOptimWrapper', can't do this
