...
param_scheduler = [
    ...
    dict(
        type='LinearLR',
        start_factor=0.001,     # Start at LR * 0.001
        by_epoch=False,         # Warmup is usually done by ITERATION, not epoch
        begin=0,                # Start at iter 0
        end=20000,                # End at iter 500
    ),
    # 2. DECAY (Multi-Step)
    dict(
        type='MultiStepLR',
        begin=0,                # Effective from the start
        end=12,                 # Total epochs (e.g., 1x schedule)
        by_epoch=True,          # Count time in Epochs
        milestones=[8, 11],     # Drop LR at epoch 8 and 11
...
log_level = 'INFO'
work_dir = None
load_from = 'work_dirs/cmt_voxel001_res_800x320_pruner/Hard_Token_Prune_Decoder_FT/epoch_8.pth'
resume = False
