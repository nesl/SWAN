...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            'camera_early_exit_model': dict(lr_mult=0.1),
            'lidar_early_exit_model': dict(lr_mult=0.1),
        }
    )
)

custom_hooks = [
    dict(type='FreezeLayersHook', train_module_names=['controller', 'lidar_early_exit_model', 'camera_early_exit_model'])
]
