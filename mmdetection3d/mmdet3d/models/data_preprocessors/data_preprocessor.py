...
@MODELS.register_module()
class DualVoxelizationPreprocessor(Det3DDataPreprocessor):
    def __init__(self, controller_voxel_type=None, controller_voxel_layer=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.controller_voxel_type = controller_voxel_type
   
        self.controller_voxel_layer = VoxelizationByGridShape(**controller_voxel_layer)
    ...
    def voxelize(self, points: List[Tensor],
        ...
        if self.controller_voxel_type == 'hard':
            voxels, coors, num_points, voxel_centers = [], [], [], []
            for i, res in enumerate(points):
                res_voxels, res_coors, res_num_points = self.controller_voxel_layer(res)
                res_voxel_centers = (
                    res_coors[:, [2, 1, 0]] + 0.5) * res_voxels.new_tensor(
                        self.controller_voxel_layer.voxel_size) + res_voxels.new_tensor(
                            self.controller_voxel_layer.point_cloud_range[0:3])
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                voxels.append(res_voxels)
                coors.append(res_coors)
                num_points.append(res_num_points)
